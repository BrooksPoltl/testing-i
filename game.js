...
function success(item){
    if (item.durability < 25 && 0 < item.enhancement < 14) {
        throw new Error('level cannot increase at this durability')
    }
    ...
    if(item.enhancement > 20){
        throw new Error('this item is max level already')
    }
    return item
}
...
function fail(item){
    if(item.enhancement <= 5){
        throw new Error('this item cannot fail')
    }
    ...
    if(item.durability < 0){
        throw new Error('durability less than 0!')
    }
